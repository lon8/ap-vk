...
def vk_kernel(search_query : str):
    ...
    friends_count = vk.friends.get(user_id=user_id)['count']
    followers_count = vk.users.getFollowers(user_id=user_id)['count']
    ...
    for i in range(0, 1000000, 100):
        wall_info : dict = vk.wall.get(count=100, filter='owner', offset=i, domain=domain)['items']
    ...
    for postnum, post_id in enumerate(post_id_list, start=0):
        ...
        comments = vk.wall.getComments(owner_id=user_id, post_id=post_id, count=100, fields='id,domain,first_name,last_name,photo_200_orig',extended=1)
    ...
    friends_info_list = vk.friends.get(user_id=user_id,fields='domain,photo_200_orig,country,city,bdate,sex', count=10000)['items']
